...
if __name__ == '__main__':
    ...
    # label_img = np.array(Image.open(path))[:, :, 0] / 255.  # 假设伪标签二值化
    # label_tensor = torch.tensor(label_img, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    #
    # ### 提取边界
    # # 不再需要 softmax 或概率 map，直接输入离散的伪标签张量
    # boundary_tensor = exrct_boundary(label_tensor, iter_=1)
    #
    # # 转为 NumPy 格式用于显示
    # boundary_np = boundary_tensor.squeeze(0).squeeze(0).cpu().numpy() * 255.
    # I = Image.fromarray(boundary_np.astype(np.uint8))
    # I.show()
